...
def Execute_instances(folder_num, folder, directory_path, number_of_smtp, number_of_send):
    ...
    for index, col in df.iterrows():
        ...
        print(rows_per_file*(folder_num-1), index, rows_per_file*folder_num)
    ...
    df2 = pd.DataFrame({'Email': Email, 'Password': Password, 'SMTP': SMTP,'From':From,
                       'PORT': PORT, 'Limit': Limit, 'No_of_send': No_of_send})
...
def instance(number_of_send, number_of_instance):
    ...
    for i in range(number_of_smtp):
        shutil.copytree("Data", "instance"+str(i+1))
        print(i)
    ...
    for folder in folders:
        t1 = threading.Thread(target=Execute_instances, args=(
            folder_num, folder, directory_path, number_of_smtp, number_of_send))
        t1.start()
        folder_num += 1
